...
// Import the base class and the KeyscopeMessage / Subscription
...
// 2. Add `Commands` to `KeyscopeClient class with`.
// 3. Export `Commands` in `lib/keyscope_client.dart`.
// ------------------------------------------------------------------------
...
import 'commands/generic/commands/expire.dart' show ExpireCommand;
...
// Re-export KeyscopeMessage from the main library file
export 'package:keyscope_client/keyscope_client_base.dart'
    show KeyscopeMessage, Subscription;
...
class _BufferReader {
  ...
  Uint8List? readBytes(int length) {
    if (length < 0) {
      throw KeyscopeParsingException('Invalid RESP length: $length');
    }
...
class KeyscopeClient // FYI. extends KeyscopeConnection
    with
    ...
    implements
        KeyscopeClientBase {
  static final _log = KeyscopeLogger('KeyscopeClient');
  ...
  ///
  /// By default, logging is `KeyscopeLogLevel.off`.
  /// Use `KeyscopeLogLevel.info` or `KeyscopeLogLevel.warning` for debugging
  /// connection or parsing issues.
  static void setLogLevel(KeyscopeLogLevel level) {
    KeyscopeLogger.level = level;
  }
  ...
  // final KeyscopeConnectionSettings _config;
  ...
  KeyscopeConnectionSettings? get currentConnectionConfig => _config;
  ...
  StreamController<KeyscopeMessage>? _pubSubController;
  ...
  ///
  /// Used by [KeyscopePool] to determine if a connection should be discarded
  /// instead of reused.
  bool get isStateful {
  ...
  /// response before throwing a [KeyscopeClientException].
  KeyscopeClient({
  ...
  })  : _config = KeyscopeConnectionSettings(
          host: host,
          port: port,
          username: username,
          password: password,
  ...
  // KeyscopeClient.fromSettings(this._config);

  /// Creates a client using a [KeyscopeConnectionSettings] object.
  factory KeyscopeClient.fromSettings(KeyscopeConnectionSettings settings) =>
      KeyscopeClient(
        host: settings.host,
        port: settings.port,
  ...
  Future<void> get onConnected =>
      ...
      Future.error(KeyscopeClientException(
  ...
  Future<void> connect({
    ...
    try {
      ...
      final connEx = KeyscopeConnectionException(
  ...
  Future<void> _addReplicaFromConfig(
      KeyscopeConnectionSettings settings) async {
    // 1. Deduplication
    ...
    try {
      ...
      final replicaClient = KeyscopeClient.fromSettings(safeSettings);

      // TODO: REVIEW REQUIRED => REMOVE. NEED CONSENSUS.
      // final replicaClient = KeyscopeClient(
      //   host: settings.host,
      //   port: settings.port,
      //   username: settings.username,
      //   password: settings.password,
  ...
  //     // 3. Connect to the replica
  //     final replicaClient = KeyscopeClient.fromSettings(replicaSettings);
  //     await replicaClient.connect();
  ...
  Future<void> _initializeConnection() async {
    ...
    if (_config.database > 0) {
      ...
      // TODO: change to keyscope_client.infoSelect()
      // Use _executeInternal to bypass routing during initialization
    ...
    while (_buffer.isNotEmpty) {
      ...
      try {
        ...
        if (reader._offset == initialOffset) {
          throw KeyscopeParsingException('Parser completed but did not advance '
  ...
  void _handlePubSubMessage(List<dynamic> messageArray) {
    ...
    if (type == 'smessage' && messageArray.length == 3) {
      ...
      if (_pubSubController != null && !_pubSubController!.isClosed) {
        _pubSubController!
            .add(KeyscopeMessage(channel: channel, message: message ?? ''));
      }
    }
    ...
    // 3. Sharded Unsubscribe Confirmation (sunsubscribe)
      ...
      if (type == 'message' && messageArray.length == 3) {
        ...
        if (_pubSubController != null && !_pubSubController!.isClosed) {
          _pubSubController!.add(KeyscopeMessage(
              channel: channel,
      ...
      // and DO NOT complete the command completer from the queue.
        ...
        _pubSubController?.addError(KeyscopeClientException(
          ...
          if (!completer.isCompleted) {
            ...
            if (isError) {
              completer.completeError(
                  result ?? KeyscopeException('Unknown error'), stackTrace);
            } else {
    ...
    try {
      ...
      switch (responseType) {
        ...
        case 45: // '-' Error
          ...
          result = KeyscopeServerException(
              line); // Return as KeyscopeServerException
          break;
        case 36: // '$' Bulk String
          final line = reader.readLine();
          if (line == null) {
          ...
          if (dataLength == -1) {
            ...
            // Handle binary data (e.g., DUMP command response) safely.
            try {
              // Attempt to decode as UTF-8 string
              result = utf8.decode(data);
            } catch (e) {
              // If decoding fails (invalid UTF-8), return the raw byte array
              // (List<int>)
              result = data;
            }
          }
        ...
        default:
          ...
          result = KeyscopeParsingException(
      ...
      if (e is KeyscopeException || e is _IncompleteDataException) rethrow;
      // Wrap other parsing errors (e.g., int.parse)
      throw KeyscopeParsingException('Failed to parse RESP response: $e');
    }
  }
  ...
  void _resolveNextCommand(dynamic response,
    ...
    // Check for *any* Exception, not just KeyscopeConnectionException
    ...
    if (_isAuthenticating) {
      ...
      if (!_connectionCompleter!.isCompleted) {
        if (responseIsError) {
          ...
          // Wrap auth error (using KeyscopeServerException if possible)
          final message = response is KeyscopeServerException
              ? response.message
              : response.toString();
          final authError = KeyscopeConnectionException(
  ...
  @override
  Future<dynamic> execute(List<dynamic> command) async {
    final cmdName =
        command.isNotEmpty ? command[0].toString().toUpperCase() : '';
  ...
  ///
  /// **Changes**
  ///   - Changed `List<String>` to `List<dynamic>` to support
  ///     binary data (e.g., RESTORE).
  ///   - Removed @override -> Renamed original 'execute' logic
  ///     to '_executeInternal'.
  ///
  Future<dynamic> _executeInternal(List<dynamic> command) async {
    // command[0] can be dynamic, so strictly convert to String for checking
    final cmdUpper =
        command.isNotEmpty ? command[0].toString().toUpperCase() : '';
    ...
    //   e.g., "KeyscopeClientException: Cannot execute command SUNSUBSCRIBE while in Pub/Sub mode..."
    ...
    if (_isInPubSubMode) {
      ...
      if (!allowedCommands.contains(cmdUpper)) {
        ...
        return Future.error(KeyscopeClientException(
    ...
    // Byte builder
    //
    // Use BytesBuilder instead of StringBuffer to handle mixed String/Binary data
    final builder = BytesBuilder();

    // Add Array Header: *<count>\r\n
    builder.add(utf8.encode('*${command.length}\r\n'));

    for (final arg in command) {
      List<int> bytes;

      // Check if argument is already binary (List<int>)
      if (arg is List<int>) {
        bytes = arg; // Use raw bytes directly (Binary Safe)
      } else {
        bytes = utf8.encode(arg.toString()); // Convert to String -> UTF-8
      }

      // Add Bulk String Header: $<length>\r\n
    ...
    try {
      if (_socket == null) {
        throw KeyscopeConnectionException('Client not connected.');
      }
      ...
      // Write raw bytes (builder.toBytes()) instead of
      // string (buffer.toString()).
      //
      // ( FYI. Previously, _socket!.write(buffer.toString()); )
      //
      _socket!.add(builder.toBytes());
    } catch (e, s) {
      // Wrap write error
      final writeError = KeyscopeConnectionException(
      ...
      if (completer != null &&
        ...
        final error =
            KeyscopeConnectionException('Failed to send $cmdUpper command: $e');
    ...
    if (cmdUpper == 'SUBSCRIBE') {
      return _subscribeReadyCompleter?.future ??
          Future.error(
              KeyscopeClientException('Subscribe completer not initialized'));
    }
    if (cmdUpper == 'PSUBSCRIBE') {
      return _psubscribeReadyCompleter?.future ??
          Future.error(
              KeyscopeClientException('PSubscribe completer not initialized'));
    }
  ...
  // TODO: multi-key for exists (e.g., cluster/shard)
  @override
  ...
  Future<int> expire(
    ...
    bool lt = false,
      ...
      ExpireCommand(this).expire(key, seconds, nx: nx, xx: xx, gt: gt, lt: lt);
  ...
  Subscription subscribe(List<String> channels) {
    ...
    if (_pubSubController == null || _pubSubController!.isClosed) {
      _pubSubController = StreamController<KeyscopeMessage>.broadcast();
    }
  ...
  Subscription psubscribe(List<String> patterns) {
    if (_isInPubSubMode && _subscribedChannels.isNotEmpty) {
      ...
      // throw KeyscopeClientException('Mixing channel and pattern subscriptions
  ...
  Future<List<ClusterSlotRange>> clusterSlots() async {
    try {
      ...
      // Re-throw as a KeyscopeException if it's not one already
      if (e is KeyscopeException) rethrow;
      // Wrap unknown errors
      throw KeyscopeClientException('Failed to execute CLUSTER SLOTS: $e');
    }
  }
  ...
  void _handleSocketError(Object error, StackTrace stackTrace) {
    final connEx =
        KeyscopeConnectionException('Socket error occurred: $error', error);
  ...
  void _sendAuthCommand(String password, {String? username}) {
    ...
    try {
      ...
      final authError =
          KeyscopeConnectionException('Failed to send AUTH command: $e', e);
